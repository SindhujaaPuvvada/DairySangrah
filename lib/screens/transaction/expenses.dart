...
class _AddExpensesState extends State<AddExpenses> {
  ...
  Future<void> _getFeedData() async {
    for (var fdType in fdCategoryId) {
      final snapshot = await fdDB.infoFromServerForCategory(fdType);

      setState(() {
        List<Feed> feeds =
            snapshot.docs
                .map((doc) => Feed.fromFireStore(doc, fdType))
                .toList();
  ...
  Future<void> _addExpense(Expense data) async {
    await dbExpense
        .infoFromServerExpenseOnDate(data.name, data.expenseOnMonth)
        .then((doc) async {
          if (doc.exists) {
  ...
  Widget build(BuildContext context) {
    ...
    Map<String, String> categoryMap = {
      ...
      'Equipment and Machinery':
          currentLocalization['Equipment and Machinery']!,
                    ...
                    children: [
                      Padding(
                        ...
                        child: TextFormField(
                          ...
                          decoration: InputDecoration(
                            labelText:
                                '${currentLocalization['date_of_expense']}',
                            hintText: 'YYYY-MM-DD',
                      ...
                      Padding(
                        ...
                        child: TransUtils.buildDropdown(
                          label:
                              '${currentLocalization['select_expense_type']}*',
                          value:
                      ...
                      if (_selectedCategory == 'Feed') ...[
                                      ...
                                      child: Column(
                                        ...
                                        children: <Widget>[
                                          if (_feedTypes.isEmpty) ...[
                                            Text(
                                              currentLocalization['out_of_stock'] ??
                                                  '',
                                              style: TextStyle(
                                                fontSize: 18,
                                                fontWeight: FontWeight.bold,
                                                ...
                                                children: [
                                                  ...
                                                  SizedBox(
                                                    width:
                                                        ...
                                                        0.37,
                                                    child: Text(
                                                      '${currentLocalization['Type']}: ${currentLocalization[item.feedType] ?? item.feedType} |\n${currentLocalization['Quantity']}: ${item.quantity} ${currentLocalization['Kg']} |\n${currentLocalization['Rate']}: ₹${item.ratePerKg} / ${currentLocalization['Kg']}',
                                                    ),
                                                  ),
                                                  SizedBox(width: 20),
                                                  (_selectedFeed.contains(item))
                                                      ? Expanded(
                                                        child: TextFormField(
                                                          controller:
                                                          ...
                                                          decoration:
                                                              InputDecoration(
                                                                labelText:
                                                                    currentLocalization['enter_qty_consumed_kg'] ??
                                                                    '',
                                                              ),
                                                          validator: (value) {
                                                            if (value == null ||
                        ...
                        Container(
                          ...
                          child: TransUtils.buildElevatedButton(
                      ...
                      Padding(
                        ...
                        child: TransUtils.buildTextField(
                          ...
                          _selectedCategory == 'Feed' ? true : false,
                        ),
                      ),
  ...
  Future<void> _submitExpense() async {
    if (_formKey.currentState!.validate()) {
      final data = Expense(
        name:
            (_selectedCategory.toString() != 'Other')
      ...
      if (mounted) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => const TransactionPage(showIncome: false),
          ),
        );
      }
    }
