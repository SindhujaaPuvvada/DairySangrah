...
class CattleUtils {
  ...
  static Widget buildTextField(TextEditingController controller, String label,
    ...
    return TextFormField(
      ...
      inputFormatters: allowNumOnly == true ? [FilteringTextInputFormatter.allow(RegExp(r"[0-9]"))]: null,
  ...
  static Widget buildReadonlyTextField({required String initialValue, required String label}){
    return TextFormField(
      readOnly: true,
      enabled: false,
  ...
  static Widget buildReadonlyTextFieldWithController({required TextEditingController controller, required String label}){
    return TextFormField(
      readOnly: true,
      enabled: false,
      style:  TextStyle(fontSize: 15, color: Colors.black),
  ...
  static Future<String> addCattleGrouptoDB(String cattleType, String breed, String status) async {
    String uid = FirebaseAuth.instance.currentUser!.uid;
    DatabaseServicesForCattleGroups cgrpDB = DatabaseServicesForCattleGroups(uid);

    int lastGrpId = int.parse(await cgrpDB.getLastUsedGrpId(uid));
    bool grpAlrdyExists = await cgrpDB.grpCriteriaExists(cattleType,breed,status);
